// Renders brand/*.svg to PNG with the site's fonts and inlined stock logos.  Run:  node brand/render.mjs
// In an SVG, write {{LOGO:NVDA}} inside an href attribute and it becomes a data URI of logos/NVDA.png.
...
for (const [src, out, width] of jobs) {
  const svg = readFileSync(src, "utf8").replace(/\{\{LOGO:([A-Z]+)\}\}/g, (_, t) => logo(t));
